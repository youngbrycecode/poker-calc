...
TEST(DeckTests, SufficientShuffleRandomness)
{
   ...
   constexpr int numOccurrencesThreshold = expectedNumOccurrences * 0.05;
...
TEST(DeckTest, ShufflePerformanceTest)
{
   std::cout << "Starting shuffle performance test: \n";

   Deck deck;
   Random random(0);

   const int numShuffles = 1e7;
   ...
   std::cout << (static_cast<double>(numShuffles)) / totalTimeSeconds << " Shuffles per second\n";
}
